...
<?php include 'header.php';?>
    <title>版本列表-<?php echo $title?></title>
...
<?php include 'overview.php';?>
 ...
 $query = "select count(*) as total from version";
...
if($_curPageNum < $_pageNum && $_end < $pages){
 ...
 ?>
 									<?php 
										$sql="select * from version ORDER BY `id` DESC limit $offset,$page_size";
										$result=mysql_query($sql);
										if($result&&mysql_num_rows($result)){
											while ($row=mysql_fetch_assoc($result)) {
											echo "<tr class='gradeX'>";
											echo "<td >".$row['name']."</td>";
											echo "<td >".base64_decode($row['content'])."</td>";
											echo "<td >".$row['file']."</td>";
											echo "<td >
												<a class='btn btn-primary btn-xs' href='version-chanage.php?id=".$row['id']."'><i class='fa fa-edit'></i></a>   <a class='btn btn-primary btn-xs' href='version-del.php?id=".$row['id']."'><i class='fa fa-ban'></i></a>
											</td>";
											echo "</tr>";
											}
										}
